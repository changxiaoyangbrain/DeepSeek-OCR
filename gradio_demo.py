...
def _ensure_offline_env():
    """Set environment variables to enforce offline mode and avoid telemetry."""
    os.environ.setdefault("HF_HUB_OFFLINE", "1")
    os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")
    os.environ.setdefault("HF_HUB_DISABLE_TELEMETRY", "1")
    # Enable efficient local blob handling if present
    os.environ.setdefault("HF_HUB_ENABLE_HF_TRANSFER", "1")

def load_model():
    """Load the DeepSeek-OCR model and tokenizer (offline-first)."""
    global model, tokenizer

    if model is None:
        print("Loading DeepSeek-OCR model (offline-first)...")
        ...
        if not local_dir:
            raise RuntimeError(
                "No local HF snapshot found for deepseek-ai/DeepSeek-OCR. "
                "Please pre-download the model or ensure the snapshot exists under "
                "~/.cache/huggingface/hub/models--deepseek-ai--DeepSeek-OCR/snapshots."
            )

        # Verify essential files exist in local snapshot
        required_files = [
            "config.json",
            "model-00001-of-000001.safetensors",
            "tokenizer.json",
        ]
        missing = [f for f in required_files if not os.path.exists(os.path.join(local_dir, f))]
        if missing:
            raise RuntimeError(
                f"Local snapshot missing required files: {missing}. "
                "Ensure the snapshot contains symlinks to blobs and tokenizer assets."
            )

        model_name = local_dir

        # Load tokenizer and model strictly from local files
        ...
        print(f"Model loaded successfully from local snapshot: {model_name}")
